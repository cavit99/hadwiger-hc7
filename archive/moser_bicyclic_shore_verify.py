#!/usr/bin/env python3
"""Independent replay of all six-vertex bicyclic shore certificates."""
...
def atlas_types():
    ...
    for graph in nx.graph_atlas_g():
        if len(graph) != 6 or graph.number_of_edges() != 7 or not nx.is_connected(graph):
            continue
...
with open("moser_bicyclic_shore_certificate.json", encoding="utf-8") as source:
    archive = json.load(source)

assert archive["format"] == 1 and archive["edge_count"] == 7
assert len(archive["cases"]) == 19
...
assert total_models == 28575
print(f"verified 19 bicyclic quotient types and {total_models} K6 model clauses")
print("all bicyclic-shore counterexample formulas: UNSAT")
