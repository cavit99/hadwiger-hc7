#!/usr/bin/env python3
"""Replay all two-vertex/contracted-shore portal certificates."""
...
def edges_for(defects):
    edges = set(M)
    edges.update({(A, W), tuple(sorted(DA))})
    ...
    edges.update(tuple(sorted((DB, s))) for s in portals | {B})
    return edges
...
with open("portal_k2k1_certificate.json", encoding="utf-8") as source:
    archive = json.load(source)

assert archive["format"] == 1
expected = set(itertools.product((None,) + U + (W,), repeat=2))
seen = set()
...
assert len(seen) == 7 ** 2
print(f"verified {len(seen)} K2/K1 defect certificates")
