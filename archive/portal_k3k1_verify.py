#!/usr/bin/env python3
"""Replay the K3/opposite-shore model-or-six-cut certificate."""
...
def valid(rows, aw):
    if any(row.bit_count() < 5 for row in rows):
        return False
...
def edges_for(rows, aw):
    edges = set(M)
    edges.update(tuple(sorted(edge)) for edge in itertools.combinations(DA, 2))
    if aw:
        edges.add((A, W))
...
with open("portal_k3k1_certificate.json", encoding="utf-8") as source:
...
for record in archive["records"]:
    ...
    if record.get("survivor"):
        ...
        survivors_seen.add(key)
        rows = key[1]
        # Every listed exception has an adjacent pair with identical row
        # 115.  Its open neighbourhood is the third triangle vertex plus
        # the five labels in that row, hence is a forbidden six-cut.
        assert rows.count(115) == 2
        assert (115).bit_count() == 5
        continue
...
assert survivors_seen == SURVIVORS
assert len(seen) == 5051
print(f"verified {len(seen)} canonical K3/K1 cases; four end in a six-cut")
